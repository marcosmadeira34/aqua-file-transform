...
const ROICalculator = () => {
  ...
  const calculateROI = () => {
    ...
    const manualTimePerPdf = 5; // 5 minutos por PDF manual
    const automatedTimePerPdf = 0.5; // 30 segundos com automação
    
    const monthlySavingMinutes = pdfs * (manualTimePerPdf - automatedTimePerPdf);
    const monthlySavingHours = monthlySavingMinutes / 60;
    const monthlyCostSaving = monthlySavingHours * (cost / 160); // assumindo 160h/mês
    
    const annualSaving = monthlyCostSaving * 12;
    const systemCost = 297; // custo mensal do sistema
    const annualSystemCost = systemCost * 12;
    ...
    setResults({
      ...
      roi: roi.toFixed(0),
      paybackMonths: (annualSystemCost / monthlyCostSaving).toFixed(1)
    });
  };
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="monthly-pdfs">
                Quantos PDFs você processa por mês?
              </Label>
              <Input
                id="monthly-pdfs"
                type="number"
            ...
            <div className="space-y-2">
              <Label htmlFor="employee-cost">
                Custo médio por hora do colaborador (R$)
              </Label>
              <Input
                id="employee-cost"
                type="number"
